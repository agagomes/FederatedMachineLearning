...
def test_merge_amrules():
    ...
    ### split based on target, sequentially
    ...
    if not ARTIFICIAL_DF:
        ...
        #usedcars and diamonds
        df = df.sort_values('price') # sort by target
        #NOMINAL_ATTRIBUTES = ['region',''manufacturer','fuel','title_status','transmission','drive','type','state']
        

        """
        #traffic
        df = df.sort_values('Deaths') # sort by target
        #NOMINAL_ATTRIBUTES = ['Sidedness','Historical_Population','Entity','Year']
        """
